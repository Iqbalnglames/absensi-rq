  ...
  <div class="flex h-screen">
   ...
   {{ Route::is('kesantrian.*') ? 'bg-blue-500 text-white hover:bg-blue-600' : 'text-gray-700 hover:bg-gray-100' }}">
          ...
          <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="block px-4 py-2 rounded-lg hover:bg-gray-100 text-red-500">Logout</button>
          </form>
        </nav>
    </aside>
      ...
      <header class="bg-white shadow-sm">
        <div class="flex justify-between items-center px-6 py-4">
          ...
          <div class="flex items-center space-x-3">
            <span class="text-sm text-gray-600">{{ Carbon::now()
  ->locale('id')
  ->translatedFormat('l, d M Y') }}</span>
