...
@section('content')
    <div>
        ...
        <div class="flex justify-between mb-4">
            <h2 class="text-xl font-semibold">Data Absensi Siswa</h2>
        </div>
        @if(session('error'))
                        ...
                        <tr class="border-b border-gray-200 hover:bg-gray-50">
                            ...
                            <td class="p-4">
                                <div class="flex gap-2 items-center">
                                    <div>
                                        <p>Hadir {{ $s->absen_murid->where('status', 'hadir')->count() }}</p>
                                        <p>Sakit {{ $s->absen_murid->where('status', 'sakit')->count() }}</p>
                                        <p>Alpha {{ $s->absen_murid->where('status', 'alpha')->count() }}</p>
                                    </div>
                                    <div>
                                        <a href="{{ route('kurikulum.absen-siswa.detail', $s->id) }}" class="text-blue-600 hover:text-blue-800">Detail</a>
                                    </div>
                                </div>
                                </td>
                            <td class="p-4">
