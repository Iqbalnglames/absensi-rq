...
@section('content')
    <a href="{{ route('kesantrian.ekskul') }}" class="text-sm text-blue-500 mb-4 inline-block">
        ← Kembali
    </a>
    ...
    <div class="flex flex-col justify-center gap-10">
        <div class="bg-white shadow rounded-xl overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody>
                    @forelse($detailEkskul->murid->sortBy('nama') as $s)
                        <tr class="border-b border-gray-200 hover:bg-gray-50">
                            <td class="p-4">{{ $s->nama }}</td>
                            <td class="p-4">Kelas {{ $s->kelas->nama_kelas }}</td>
                            <td class="p-4">
                                <form method="POST" action="{{ route('kesantrian.ekskul.deletePeserta', [$detailEkskul->id, $s->id]) }}">
                                    @csrf
                                    @method('DELETE')
