...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        <form action="{{ route('kesantrian.perizinan.update', $perizinan->id) }}" method="POST" class="bg-white shadow rounded-xl p-6 space-y-4">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <input name="tanggal" type="date" value="{{ $perizinan->tanggal }}" class="w-full border rounded-lg px-3 py-2" required>
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Waktu mulai izin</label>
                <input name="waktu_mulai_izin" type="time" value="{{ explode('-', $perizinan->waktu_izin)[0] }}" class="w-full border rounded-lg px-3 py-2" required>
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Waktu selesai izin</label>
                <input name="waktu_selesai_izin" type="time" value="{{ explode('-', $perizinan->waktu_izin)[1] }}" class="w-full border rounded-lg px-3 py-2" required>
            </div>
